...
function fetchSearchResults(query) {
  const url = `${API_BASE_URL}/search/movie?api_key=${API_KEY}&language=ko-KR&query=${encodeURIComponent(
    query
  )}&page=${currentPage}`;
  fetchMoviesFromAPI(url);
}
...
async function fetchAndDisplayMovieDetails(movieId) {
  try {
    const [movieDetails, credits] = await Promise.all([
      ...
      fetch(`${API_BASE_URL}/movie/${movieId}/credits?api_key=${API_KEY}`).then(
        (res) => res.json()
      ),
    ]);
...
function displayMovieModal(movie) {
  modalContent.innerHTML = `
    <div class="modal-backdrop" style="background-image: url('${IMAGE_BASE_URL}w1280${
    movie.backdrop_path
  }');">
    ...
    movie.title
          ...
          <div class="modal-info">
            <div class="modal-rating">
              <div class="rating-circle" style="--rating: ${
